...
HANDLE hReadyEvent, hConnetEvent; // 이벤트
...
DWORD WINAPI ProcessTime(LPVOID arg)
{
	...
	WaitForSingleObject(hConnetEvent, INFINITE); // 클라 접속 완료 기다리기
...
DWORD WINAPI ProcessClient(LPVOID arg)
{
	...
	while(1){
		WaitForSingleObject(hReadyEvent, INFINITE); // 게임 준비 상태 기다리기
		...
		SetEvent(hConnetEvent); // 클라 정보 받기 완료
	}
